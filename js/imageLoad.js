	...
	sortObjectArray(logoArray, "title").forEach(logo => {
		...
		const html =
			...
			`<div class='logo'> <div class='logo__holder'> <div class='logo__image'> <img loading="lazy" src='logos/${
				...
				logo.title
			} Logo.png'> <span class='logo__download__overlay--png'> Download PNG </span> </a> </div></div></div><div class='logo__text'> <p class='logo__text--primary'>${
				logo.title
			}</p><p class='logo__text--secondary'>${(logo.category + "").replace(
				",",
				" / "
			)}</p></div></div>`;
